...
const datatable = new Datatable('#tablaPermisos', {
    ...
    columns: [
        ...
        
        {
            ...
            orderable: false,
            render: (data, type, row, meta) => {
                if (row['usu_estado'].trim() === 'ACTIVO') {
                    return `<button class="btn btn-success" data-id="${data}" data-estado="ACTIVO">ACTIVO</button>`;
                } else if (row['usu_estado'].trim() === 'PENDIENTE' || row['usu_estado'].trim() === 'INACTIVO') {
                    return `<button class="btn btn-info" data-id="${data}" data-estado="INACTIVO">INACTIVO</button>`;
                } else {
                    return '';
                }
            }
...
const buscar = async () => {
    ...
    const url = `/examenparcial/API/permisos/buscar?permiso_usuario=${permiso_usuario}&permiso_rol=${permiso_rol}`;
    const config = {
        method : 'GET'
    }
    try {
...
const guardar = async (evento) => {
    ...
    const url = '/examenparcial/API/permisos/guardar';
...
const modificar = async () => {
    ...
    const body = new FormData(formulario)
    const url = '/examenparcial/API/permisos/modificar';
    const config = {
        method : 'POST',
        body
    }
...
const eliminar = async (e) => {
    ...
    if (await confirmacion('warning', '¿Desea eliminar este registro?')) {
        ...
        body.append('permiso_id', id)
        const url = '/examenparcial/API/permisos/eliminar';
        const config = {
            method: 'POST',
            body
        }
...
const activar = async (e) => {
    ...
    if(await confirmacion('warning','¿Desea activar este usuario?')){
        ...
        const url = '/examenparcial/API/permisos/activar';
        const headers = new Headers();
        headers.append("X-Requested-With","fetch");
        const config = {
            method : 'POST',
...
const desactivar = async (e) => {
    ...
    if(await confirmacion('warning','¿Desea desactivar este usuario?')){
        const body = new FormData()
        body.append('usu_id', id)
        const url = '/examenparcial/API/permisos/desactivar';
        const headers = new Headers();
        headers.append("X-Requested-With","fetch");
        const config = {
            method : 'POST',
